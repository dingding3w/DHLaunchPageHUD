...
#define DDAnimationDelayTime    1.0 // 动画延迟时间
...
+ (void)dh_initWithFrame:(CGRect)frame window:(UIWindow *)window image:(UIImage *)image animationStyle:(DDLaunchPageAnimationStyle)style {
    ...
    if (!style) {
        [self dh_setLaunchPageAnimationStyle:DDLaunchPageAnimationStyleFadeOut window:window];
    } else {
        [self dh_setLaunchPageAnimationStyle:style window:window];
    }
}

+ (void)dh_setLaunchPageAnimationStyle:(DDLaunchPageAnimationStyle)animationStyle window:(UIWindow *)window{
